...
#ok#ok
  def transferir(monto, origen, destino)
  	trx= JSON.parse(HTTP.headers(:"Content-Type" => "application/json").put("http://moto.ing.puc.cl/banco/trx", :json => {:monto => monto, :origen => origen, :destino => destino} ).to_s, :symbolize_names => true)
    return trx
  end
