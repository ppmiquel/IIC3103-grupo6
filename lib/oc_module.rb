module OcModule
  ...
  def crearOC(canal, cantidad, sku, cliente,  proveedor, precio, notas, fecha)
    ...
    oc = JSON.parse(HTTP.headers(:"Content-Type" => "application/json").put("http://moto.ing.puc.cl/oc/crear", :json => {:canal => canal , :cantidad => cantidad, :sku => sku, :cliente => cliente, :proveedor => proveedor,:precioUnitario => precio, :notas => notas, :fechaEntrega => fecha}), :symbolize_names => true)
    return oc
